...
function Dropdown<T extends { id: number; name: string }>({
  label, items, selected, onSelect, renderSub,
}: {
  label: string; items: T[]; selected: T | null;
  onSelect: (item: T) => void; renderSub?: (item: T) => string;
}) {
...
export default function CreateOrderScreen({ navigation }: any) {
  ...
  const addToCart = () => {
    ...
    if (!unitPrice || unitPrice <= 0) { Alert.alert("خطأ", "أدخل سعراً صحيحاً"); return; }
    ...
    setCart(prev => {
      ...
      return [...prev, { productId: selectedProduct.id, productName: selectedProduct.name, quantity: q, price: unitPrice, deliveryFeePerTon: fee, totalDelivery: itemTotalDelivery }];
    });
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
              ...
              {parseFloat(price) > 0 && parseInt(qty) > 0 && (
                <View style={styles.previewRow}>
                  ...
                  <Text style={styles.previewValue}>
                    {((parseFloat(price) || 0) * (parseInt(qty) || 0) + (parseFloat(deliveryFeePerTon) || 0) * (parseInt(qty) || 0)).toLocaleString("ar-EG")} {CURRENCY}
                  </Text>
                </View>
              )}
            ...
            <View style={styles.section}>
                ...
                <View key={item.productId} style={styles.cartCard}>
                  <View style={styles.cartCardHeader}>
                    <TouchableOpacity
                      ...
                      onPress={() => setCart(prev => prev.filter(i => i.productId !== item.productId))}
                    >
                  ...
                  <View style={styles.cartCardBody}>
                    ...
                    {item.deliveryFeePerTon > 0 && (
                      <>
                        <View style={styles.cartDetailDivider} />
                        <View style={styles.cartDetailRow}>
                          <Text style={styles.cartDetailValue}>{item.deliveryFeePerTon} {CURRENCY}</Text>
              ...
              <View style={styles.totalCard}>
                <View style={styles.totalRow}>
                  <Text style={styles.totalSubValue}>{totalProductsAmount.toLocaleString("ar-EG")} {CURRENCY}</Text>
                  <Text style={styles.totalSubLabel}>إجمالي المنتجات</Text>
                </View>
                {totalDeliveryAmount > 0 && (
                  <View style={styles.totalRow}>
                    <Text style={styles.totalSubValue}>{totalDeliveryAmount.toLocaleString("ar-EG")} {CURRENCY}</Text>
                    <Text style={styles.totalSubLabel}>إجمالي الناولون</Text>
                  </View>
                )}
                <View style={styles.totalDivider} />
                <View style={styles.totalRow}>
                  <Text style={styles.totalFinalValue}>{total.toLocaleString("ar-EG")} {CURRENCY}</Text>
          ...
          >
            <Text style={styles.submitBtnText}>
              {saving ? "جاري الحفظ..." : `✅ تأكيد الطلب  •  ${total.toLocaleString("ar-EG")} ${CURRENCY}`}
            </Text>
          </TouchableOpacity>
...
const dd = StyleSheet.create({
  ...
  list: {
    ...
    borderColor: COLORS.primary, borderBottomLeftRadius: 10, borderBottomRightRadius: 10, overflow: "hidden",
  },
...
const styles = StyleSheet.create({
  ...
  itemFormCard: {
    ...
    marginBottom: 8, borderWidth: 1.5, borderColor: COLORS.primary + "44",
  ...
  previewRow: {
    ...
    backgroundColor: COLORS.primary + "14", borderRadius: 8, padding: 10, marginBottom: 10,
  },
  ...
  submitBtn: {
    ...
    marginTop: 4, shadowColor: COLORS.primary,
